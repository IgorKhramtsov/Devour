...
class _FakePhoneInputWidgetState extends State<FakePhoneInputWidget> with TickerProviderStateMixin {
  ...
  void initState() { 
    ...
    _cursorAnimator = AnimationController(vsync: this, duration: Duration(milliseconds: 800));
    _cursorAnimation = ReverseAnimation(CurvedAnimation(parent: _cursorAnimator, curve: Curves.easeInOutSine));
  ...
  Widget build(BuildContext context) {
    final fakeInputDecoration = BoxDecoration(
      border: Border(
        bottom: BorderSide(
          color: _focused ? Theme.of(context).accentColor : Theme.of(context).dividerColor,
          width: 2.0
        )
      )
    );
      ...
      child: AnimatedContainer(
        ...
        child: RichText(
          text: TextSpan(
            ...
            text: _kFakeEmptySymbol + getPhoneFormattedString(widget.textEditingController.text),
...
List _kSeparatorIntervals = [3, 3, 2, 2]; // ingoring the first one
String getPhoneFormattedString(final String phoneNumber) {
  ...
  for (final interval in _kSeparatorIntervals) {
    if (charactersIterator.length > 0) {
      formattedPhone += '-';
      formattedPhone += '${charactersIterator.take(interval)}';
      charactersIterator = charactersIterator.skip(interval);
    }
    else if (charactersIterator.length < interval) 
      break;
